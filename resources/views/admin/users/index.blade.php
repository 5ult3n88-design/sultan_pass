@extends('layouts.dashboard', [
    'title' => __('Staff & Administrators'),
    'subtitle' => __('Manage admin, manager, and assessor accounts'),
])
...
@section('content')
    <div class="mb-6 space-y-4">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('admin.users.index') }}" class="text-xs text-uae-gold-300 hover:text-uae-gold-100">
                {{ __('Clear all') }}
            </a>
        </div>
        <form method="GET" action="{{ route('admin.users.index') }}" class="grid gap-4 rounded-xl border border-white/10 bg-white/5 p-4 lg:grid-cols-4">
            <div>
                <label class="mb-1 block text-xs font-semibold text-slate-400">{{ __('Role') }}</label>
                <select name="role" class="w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-slate-200 focus:border-uae-gold-300 focus:outline-none focus:ring-2 focus:ring-uae-gold-300/40">
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route('admin.users.import') }}" class="inline-flex items-center gap-2 rounded-lg border border-white/10 bg-white/5 px-4 py-2 text-sm font-semibold text-slate-200 transition hover:bg-white/10">
                    <span>📤</span>
                    {{ __('Import CSV') }}
                </a>
                <a href="{{ route('admin.users.export-csv') }}" class="inline-flex items-center gap-2 rounded-lg border border-white/10 bg-white/5 px-4 py-2 text-sm font-semibold text-slate-200 transition hover:bg-white/10">
                    <span>📥</span>
                    {{ __('Export CSV') }}
                </a>
                <a href="{{ route('admin.users.create') }}" class="inline-flex items-center gap-2 rounded-lg bg-uae-gold-300/80 px-4 py-2 text-sm font-semibold text-white shadow hover:bg-uae-gold-300">
            <span>+</span>
                    {{ __('Add staff user') }}
        </a>
            </div>
            ...
            <tbody class="divide-y divide-white/5 text-slate-100">
                @forelse($users as $user)
                    <tr>
                        ...
                        <td class="px-5 py-4">{{ $user->language?->name ?? __('Auto detect') }}</td>
                        <td class="px-5 py-4">
                        ...
                        <td class="px-5 py-4 text-right">
                            <a href="{{ route('admin.users.edit', $user) }}" class="rounded-lg border border-white/10 px-3 py-1.5 text-xs font-semibold text-slate-200 transition hover:bg-white/10">
                                {{ __('Edit') }}
                            </a>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="7" class="px-5 py-10 text-center text-sm text-slate-300">
                            {{ __('No users found for the selected filter.') }}
                        </td>
                    </tr>
                @endforelse
            </tbody>
