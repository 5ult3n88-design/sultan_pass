@extends('layouts.dashboard', [
    ...
    'subtitle' => __('View and manage assessments'),
])

@section('content')
    <div class="flex items-center justify-between mb-6">
        <form method="GET" action="{{ route('manager.assessments') }}" class="flex items-center gap-3">
        ...
        <a href="{{ route('assessments.create') }}" class="inline-flex items-center gap-2 rounded-lg bg-uae-gold-300/80 px-4 py-2 text-sm font-semibold text-white shadow hover:bg-uae-gold-300">
            <span>+</span>
                ...
                @forelse($assessments as $assessment)
                    <tr>
                        ...
                        <td class="px-5 py-4 text-right">
                            <a href="{{ route('assessments.report', $assessment) }}" class="rounded-lg border border-white/10 px-3 py-1.5 text-xs font-semibold text-slate-200 transition hover:bg-white/10">
                                {{ __('View report') }}
                            </a>
                        </td>
                    </tr>
